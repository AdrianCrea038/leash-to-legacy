...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function getCurrentSiteData() {
    try {
      ...
      const { data: blogData } = await window.supabaseClient.from('blog_posts').select('*');
      const { data: videoData } = await window.supabaseClient.from('training_videos').select('*');

      config.blog = { posts: blogData || [] };
      config.training_videos = videoData || [];
      
      return config;
    } catch (e) {
      console.warn('Falló Supabase, usando respaldo local:', e);
  ...
  async function saveConfig(id, content, message) {
    try {
      const { error } = await window.supabaseClient
        .from('site_config')
        .upsert({ id, content, updated_at: new Date() });
      
      if (error) throw error;
      alert(message + ' ✅ (Guardado en Supabase)');
      
      // Actualizar respaldo local
      const current = await getCurrentSiteData();
      localStorage.setItem('site_data_backup', JSON.stringify(current));
    } catch (e) {
  ...
  if (heroForm) {
    ...
    heroForm.addEventListener('submit', async function(e) {
      ...
      await saveConfig('hero', content, 'Hero updated');
    });
  }
  ...
  if (aboutForm) {
    ...
    aboutForm.addEventListener('submit', async function(e) {
      ...
      const textContent = aboutContentInput?.value;
      
      const currentData = await getCurrentSiteData();
      
      const content = {
        titulo: title || (currentData.about_us?.titulo || 'About Us'),
        texto: textContent || (currentData.about_us?.texto || '')
      };
      
      await saveConfig('about_us', content, 'About Us updated');
    });
  }
  ...
  if (redesForm) {
    ...
    redesForm.addEventListener('submit', async function(e) {
      ...
      await saveConfig('social', content, 'Social links updated');
    });
  }
  ...
  async function deleteBlogPost(id) {
    ...
    try {
      ...
      if (error) throw error;
      alert('Post deleted from Supabase ✅');
      loadBlogPosts(); 
    } catch (e) {
      alert('Error: ' + e.message);
    }
  ...
  if (blogForm) {
    blogForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (blogPreviewImg) { blogPreviewImg.src = ''; blogPreviewImg.style.display = 'none'; }
        const submitBtn = document.querySelector('#blog-form button[type="submit"]');
  ...
  if (boardingForm) {
    ...
    boardingForm.addEventListener('submit', async function(e) {
      ...
      const bContent = boardingContentInput?.value;
      ...
      const content = {
        ...
        texto: bContent || (currentData.boarding?.texto || ''),
        imagen: imageUrl || (currentData.boarding?.imagen || 'IMG/Boarding-Service.png')
      };
      
      await saveConfig('boarding', content, 'Boarding updated');
    });
  }
  ...
  if (puppyForm) {
    ...
    puppyForm.addEventListener('submit', async function(e) {
      ...
      const pContent = puppyContentInput?.value;
      ...
      const content = {
        ...
        texto: pContent || (currentData.puppy?.texto || ''),
        imagen: imageUrl || (currentData.puppy?.imagen || 'IMG/Puppy-Concierge.png')
      };
      
      await saveConfig('puppy', content, 'Puppy Concierge updated');
    });
  }
